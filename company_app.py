# company_app_final_clean.py
"""
Private Listing App - Clean Version
-----------------------------------
Features:
- Company & Pincode search forms
- Quick filters + table-level filters (non-duplicated)
- Pagination & page-size selection
- Dark-green match highlighting + "MATCHED_IN"
- History (last 50) + Pins (save/apply/remove)
- CSV / Excel download
- Dashboard tab for visuals
- About tab
- Password gate with st.secrets['password'] or default
- Dark-neon theme with gold/red highlights
"""
...
.search-input .stTextInput>div>div>input { background: rgba(255,255,255,0.04) !important; border: 1px solid #FFD700 !important; color: #87CEFA !important; border-radius: 10px !important; padding: 10px !important; }
...
@media (max-width: 600px) { .stButton>button { padding: 8px 10px !important; font-size: 14px !important; } }
</style>
...
class HistEntry:
    ...
    def row(self) -> Dict[str, Any]:
        return {"Query": self.query or "(none)", "Bank": self.bank, "Category/State": self.cat_state, "Results": int(self.results), "Scope": self.scope}
...
def download_buttons(df: pd.DataFrame, csv_name: str, xlsx_name: str):
    ...
    with c1: st.download_button("⬇ Download CSV", data=csv_bytes, file_name=csv_name, mime="text/csv")
    with c2: st.download_button("⬇ Download Excel", data=excel_io, file_name=xlsx_name, mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

def _normalize(s: Any) -> str: return "" if pd.isna(s) else str(s)
    ...
    if picked:
        ...
        for _ in range(rows):
            ...
            for col_ui in cols_ui:
                if idx >= len(picked): break
                cname = picked[idx]; idx += 1
                if cname in obj_cols:
                    ...
                    if sel != "All": df = df[df[cname].astype(str) == sel]
                else:
                    numeric_series = pd.to_numeric(df[cname], errors="coerce")
                    mini, maxi = float(numeric_series.min()), float(numeric_series.max())
...
with st.sidebar:
    ...
    if st.session_state.company_history: preview_rows.extend(st.session_state.company_history[:3])
    if st.session_state.pincode_history: preview_rows.extend(st.session_state.pincode_history[:3])
    if preview_rows: st.dataframe(pd.DataFrame(preview_rows), use_container_width=True, height=180)
...
if menu=="🏢 Company Listing Checker":
    ...
    with st.form("company_search_form", clear_on_submit=False):
        ...
        st.text_input("Search text (company name, bank or category):", key="company_form_q", help="Type and press Search.")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    # Quick filters
    banks = ["All"] + sorted(COMPANY_DF["BANK_NAME"].dropna().unique().tolist()) if not COMPANY_DF.empty and "BANK_NAME" in COMPANY_DF.columns else ["All"]
    cats = ["All"] + sorted(COMPANY_DF["COMPANY_CATEGORY"].dropna().unique().tolist()) if not COMPANY_DF.empty and "COMPANY_CATEGORY" in COMPANY_DF.columns else ["All"]
    ...
    with c1:
        ...
        if bank_choice!=st.session_state.company_bank: st.session_state.company_bank=bank_choice; st.session_state.company_page=0
    with c2:
        ...
        if cat_choice!=st.session_state.company_category: st.session_state.company_category=cat_choice; st.session_state.company_page=0
    with c3:
        size_choice = st.selectbox("Rows",[10,20,50,100], index=[10,20,50,100].index(st.session_state.company_page_size) if st.session_state.company_page_size in [10,20,50,100] else 1,key="company_size_main")
        if size_choice!=st.session_state.company_page_size: st.session_state.company_page_size=size_choice; st.session_state.company_page=0
    ...
    if not results.empty:
        ...
        if st.session_state.company_bank!="All": results=results[results["BANK_NAME"].astype(str)==st.session_state.company_bank]
        if st.session_state.company_category!="All": results=results[results["COMPANY_CATEGORY"].astype(str)==st.session_state.company_category]

    results = table_filters(results, key_prefix="company_table", exclude_cols=("BANK_NAME","COMPANY_CATEGORY","COMPANY_NAME"))
    if st.session_state.company_query or st.session_state.company_bank!="All" or st.session_state.company_category!="All":
        add_history("company_history",HistEntry(st.session_state.company_query,st.session_state.company_bank,st.session_state.company_category,len(results),"company"))
    ...
    if not page_df_raw.empty and q:
        ...
        for c in search_cols: display_page[c]=display_page[c].apply(lambda x: highlight_match(x,q))
        st.markdown("### Results")
        st.markdown(display_page.to_html(escape=False,index=False,classes=["full-width-table"]),unsafe_allow_html=True)
    else: st.dataframe(page_df_raw,use_container_width=True)

    p1,p2,p3,p4,p5=st.columns([1,2,2,2,1])
    with p1: 
        if st.button("⏮ First",key="company_first") and cur>0: st.session_state.company_page=0; rerun()
    with p2:
        if st.button("⬅ Prev",key="company_prev") and cur>0: st.session_state.company_page=cur-1; rerun()
    with p3: st.markdown(f"**Page {cur+1} / {pages}**")
    with p4:
        if st.button("Next ➡",key="company_next") and cur<pages-1: st.session_state.company_page=cur+1; rerun()
    with p5:
        if st.button("Last ⏭",key="company_last") and cur<pages-1: st.session_state.company_page=pages-1; rerun()

    download_buttons(results,"company_results.csv","company_results.xlsx")

    with st.sidebar:
        st.markdown("---")
        st.subheader("📌 Company Pins")
                ...
                with st.expander(f"{i+1}. {p['Query']} | {p['Bank']} | {p['Category/State']} ({p['Results']})"):
                    ...
                    with ca:
                        if st.button("Apply",key=f"apply_company_pin_{i}"):
                            ...
                            rerun()
                    with cb:
                        if st.button("Remove",key=f"remove_company_pin_{i}"): st.session_state.company_pins.pop(i); rerun()
        else: st.write("No pins yet")
...
# =============================================================================
    ...
    with st.form("pincode_search_form", clear_on_submit=False):
        st.markdown("<div class='search-input'>", unsafe_allow_html=True)
        st.text_input("Search text (pincode, location or state):", key="pincode_form_q", help="Type and press Search.")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    banks = ["All"] + sorted(PINCODE_DF["BANK"].dropna().unique().tolist()) if not PINCODE_DF.empty and "BANK" in PINCODE_DF.columns else ["All"]
    states = ["All"] + sorted(PINCODE_DF["STATE"].dropna().unique().tolist()) if not PINCODE_DF.empty and "STATE" in PINCODE_DF.columns else ["All"]
    p1,p2,p3=st.columns([2,2,1])
    with p1: bank_idx = banks.index(st.session_state.pincode_bank) if st.session_state.pincode_bank in banks else 0; bank_choice = st.selectbox("🏦 Bank", banks, index=bank_idx,key="pincode_bank_main"); 
        if bank_choice!=st.session_state.pincode_bank: st.session_state.pincode_bank=bank_choice; st.session_state.pincode_page=0
    with p2: state_idx = states.index(st.session_state.pincode_state) if st.session_state.pincode_state in states else 0; state_choice = st.selectbox("🌍 State", states, index=state_idx,key="pincode_state_main");
        if state_choice!=st.session_state.pincode_state: st.session_state.pincode_state=state_choice; st.session_state.pincode_page=0
    with p3: size_choice = st.selectbox("Rows",[10,20,50,100], index=[10,20,50,100].index(st.session_state.pincode_page_size) if st.session_state.pincode_page_size in [10,20,50,100] else 1,key="pincode_size_main");
        if size_choice!=st.session_state.pincode_page_size: st.session_state.pincode_page_size=size_choice; st.session_state.pincode_page=0
    ...
    if not results2.empty:
        ...
        if st.session_state.pincode_bank!="All": results2=results2[results2["BANK"].astype(str)==st.session_state.pincode_bank]
        if st.session_state.pincode_state!="All": results2=results2[results2["STATE"].astype(str)==st.session_state.pincode_state]

    results2 = table_filters(results2,key_prefix="pincode_table",exclude_cols=("BANK","STATE","PINCODE"))
    if st.session_state.pincode_query or st.session_state.pincode_bank!="All" or st.session_state.pincode_state!="All":
        add_history("pincode_history",HistEntry(st.session_state.pincode_query,st.session_state.pincode_bank,st.session_state.pincode_state,len(results2),"pincode"))
    ...
    if not page_df2_raw.empty and q2:
        ...
        for c in search_cols2: display_page2[c]=display_page2[c].apply(lambda x: highlight_match(x,q2))
        st.markdown("### Results")
        st.markdown(display_page2.to_html(escape=False,index=False,classes=["full-width-table"]),unsafe_allow_html=True)
    else: st.dataframe(page_df2_raw,use_container_width=True)

    p1,p2,p3,p4,p5=st.columns([1,2,2,2,1])
    with p1: 
        if st.button("⏮ First",key="pincode_first") and cur2>0: st.session_state.pincode_page=0; rerun()
    with p2:
        if st.button("⬅ Prev",key="pincode_prev") and cur2>0: st.session_state.pincode_page=cur2-1; rerun()
    with p3: st.markdown(f"**Page {cur2+1} / {pages2}**")
    with p4:
        if st.button("Next ➡",key="pincode_next") and cur2<pages2-1: st.session_state.pincode_page=cur2+1; rerun()
    with p5:
        if st.button("Last ⏭",key="pincode_last") and cur2<pages2-1: st.session_state.pincode_page=pages2-1; rerun()

    download_buttons(results2,"pincode_results.csv","pincode_results.xlsx")

    with st.sidebar:
        st.markdown("---")
        st.subheader("📌 Pincode Pins")
                ...
                with st.expander(f"{i+1}. {p['Query']} | {p['Bank']} | {p['Category/State']} ({p['Results']})"):
                    ...
                    with ca:
                        if st.button("Apply",key=f"apply_pincode_pin_{i}"): 
                            ...
                            rerun()
                    with cb:
                        if st.button("Remove",key=f"remove_pincode_pin_{i}"): st.session_state.pincode_pins.pop(i); rerun()
        else: st.write("No pins yet")
...
# =============================================================================
    ...
    st.title("📊 Dashboard")
    if COMPANY_DF.empty or PINCODE_DF.empty: st.info("Load the datasets (Excel) to see visualizations.")
    else:
        c1,c2=st.columns(2)
        with c1:
            ...
            if "BANK_NAME" in COMPANY_DF.columns:
                ...
                fig,ax=plt.subplots(); bc.plot(kind="bar",ax=ax); ax.set_ylabel("Count"); st.pyplot(fig)
            else: st.info("BANK_NAME column missing.")
        with c2:
            st.subheader("Companies by Category")
            if "COMPANY_CATEGORY" in COMPANY_DF.columns:
                cc=COMPANY_DF["COMPANY_CATEGORY"].value_counts()
                fig,ax=plt.subplots(); cc.plot(kind="pie",autopct="%1.1f%%",ax=ax); ax.set_ylabel(""); st.pyplot(fig)
            else: st.info("COMPANY_CATEGORY column missing.")
        st.markdown("---")
...
# =============================================================================
    ...
    **Usage Notes**
    - Use Streamlit Secrets for password on Streamlit Cloud (`password` in secrets).
    - Local fallback password is defined in code (`DEFAULT_PASSWORD`).
    - Ensure Excel files are present:
